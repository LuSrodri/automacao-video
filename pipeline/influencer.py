...
ESTE DESENHO SAIU E VOLTOU (2026-09-05, decisão do usuário). Entre 04/09 e
05/09 o Short tentou o caminho inverso — ElevenLabs narrando e um modelo
fazendo o lipsync em cima do MP3 pronto — para recuperar o corte de silêncio, o
ajuste de velocidade e os timestamps de graça, que são justamente o que a
consequência 1 acima custa. Três tentativas, todas medidas contra a API real, e
nenhuma entregou:

  - `wan3.0-video` no modo REFERÊNCIA (`reference_image` + `reference_audio`):
    NÃO faz lipsync. O `reference_audio` dele é referência de TIMBRE — o guia
    da Alibaba diz o uso pretendido, "extract the voice characteristics from
    Audio 1, and have the character say the following lines" —, então o modelo
    clona a voz e sincroniza a boca da fala que ELE gera a partir do TEXTO. Sem
    roteiro no prompt não sobra nada dirigindo os lábios, e a boca fica em
    idle. O áudio voltar com correlação 0,98 nunca foi prova de sincronia;
  - `wan2.7-i2v` com `first_frame` + `driving_audio`: faz lipsync de verdade,
    mas é APROXIMADO (em 21s de narração natural, 6 fechamentos claros em 16
    bilabiais) e obriga o vídeo a nascer em pedaços de 15s INDEPENDENTES, cada
    um recomeçando da foto — a emenda virava salto de pose no meio da frase.
    Chegou a ir ao ar em 2026-09-04 (commit e366a32) e foi reprovado;
  - `wan3.0-video` no modo EDIÇÃO (`reference_video` + `reference_audio`) sobre
    uma peça muda: mecanicamente funciona e preserva identidade e cena, mas a
    qualidade do lipsync variou demais entre gerações (9 fechamentos em 16 numa
    tirada, 2 ou 3 em três outras), e um A/B na MESMA peça muda mostrou que o
    prompt não era a causa.

Fora do Wan não havia porta: `videoretalk` não existe na região de Singapura
(`Model not exist`), `wan2.7-videoedit` não aceita áudio de entrada
(`Input should be 'video' or 'reference_image'`), e a série `MiniMax-H3` — a
única da MiniMax que aceita áudio de referência, já que o `H3-Max` não suporta
reference-to-video — está barrada no plano da conta (`TokenPlan or Credit does
not currently support MiniMax-H3 series models`), o que pedia recarga.

Ou seja: o áudio preso aos lábios não é um defeito deste desenho, é o PREÇO de
ter lipsync que presta. Quem quiser mexer nisso de novo, comece medindo o
lipsync — o teste que presta é o ataque de /m/ /b/ /p/, amostrado com tolerância
de dois quadros, porque o fechamento cai depois do carimbo da ElevenLabs.
"""
