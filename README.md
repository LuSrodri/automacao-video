...
| `VETO_NAO_FILMADO` | `1` | Barra o clipe que **não é live footage**: gravação de tela, slide, apresentação, cartela, motion graphics, animação, render 3D, vídeo gerado por IA, gameplay — e filmagem embrulhada em **moldura** (mockup de celular, borda decorativa, template com painel ao lado). Veto duro, sem exceção de contexto nem de formato. `0` aceita de volta a peça montada em tela cheia |
...
5. **Veto a tudo que não é live footage (2026-08-25)** — pedido direto: *"veto qualquer vídeo do X que não seja live footage. Ou seja, veta vídeos do tipo gravação de tela, slides, apresentações, molduras, e afins"*. O clipe só entra em tela cheia se uma **câmera** o filmou no mundo físico: pessoas, lugares, objetos, equipamentos, um evento acontecendo — celular na mão, câmera de estúdio, drone, câmera de segurança, imagem aérea. Sai o que **nasceu dentro de um computador** (captura de tela, slide, apresentação, cartela, motion graphics, infográfico, animação, render 3D, vídeo gerado por IA, gameplay, chamada de vídeo) e sai o que é **outra mídia remontada**: a **moldura** — filmagem de verdade posta dentro de mockup de celular ou de navegador, de borda decorativa, de template com painel de texto ao lado, ou um print com zoom e deslize por cima.

   O veto tem **duas camadas** porque uma só não cobria o pedido. O tipo `gravacao_de_tela` entrou em `TIPOS_VETADOS_CLIPE`, ao lado de `cartela_ou_manchete`. Mas o enum de `tipo_material` **não tem casa para moldura**, nem para animação, render ou vídeo de IA — tudo isso caía em `cena_real`/`outro` e passava. Então a visão ganhou o campo **`imagem_filmada`** (`midia_x.py`) e o código, o `_veto_nao_filmado` (`auditoria.py`). A nota de pertinência não resolveria: ela **premia** a peça montada, porque quanto mais o clipe é um slide sobre o assunto, mais exatamente ele "mostra o que a narração diz" — foi assim que a cartela da Linux Foundation tirou nota 5 e ocupou um Short inteiro.

   É o veto mais amplo do módulo e roda **primeiro**, antes do tipo, do texto e do movimento — sem exceção de contexto e **sem exceção de formato** (no `--long-take` também vale: telejornal só entra marcado quando é VT filmado). Vale só para os **clipes**; as cartelas passam com `vetar_nao_filmado=False`, porque a cartela mais útil do canal é o **print do post citado**, que nenhuma câmera filmou. A triagem que roda antes da escolha da pauta usa o mesmo veto, então candidata sem filmagem perde a vez de graça. Laudo antigo, sem o campo, não veta ninguém. `VETO_NAO_FILMADO=0` desliga.
