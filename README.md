...
- Chaves de API (três):
  ...
  - **X API** — Consumer Key + Secret do app em [developer.x.com](https://developer.x.com) (leitura das contas que você segue, coleta dos posts delas e download dos clipes; pay-per-use).
...
# 2. Crie o .env a partir do exemplo e preencha as chaves (e o X_USERNAME)
...
| --- | --- | --- |
| `X_USERNAME` | — | Seu handle do X (sem `@`). A coleta lê **as contas que ele segue** e é delas que sai a pauta. Obrigatório, a menos que `X_ACCOUNTS` esteja preenchido |
| `X_ACCOUNTS` | vazio | Opcional (escape hatch): usa somente estas contas, no lugar da lista de seguidas |
...
## Como funciona a leitura das contas seguidas

Pedido do usuário em **2026-08-16**: *"mude para ler as contas que eu sigo"*. Até então a pauta saía de uma **lista fixa de 192 handles** no código (`CONTAS_PADRAO` em `pipeline/config.py`), curada e verificada uma a uma contra a X API em 2026-07-30. Mexer nela exigia commit e deploy.

Agora o pipeline lê **`/2/users/:id/following`** do handle em `X_USERNAME` a cada execução (`x_client.contas_seguidas`). **Seguir ou deixar de seguir alguém no X passou a ser a forma de mexer na pauta do canal** — a mudança vale já na execução seguinte, sem tocar no código.

- **Custo**: uma requisição. O teto por página é 1.000 contas, então as ~170 que o usuário segue cabem numa só. A resposta traz `id` e `username` juntos, o que ainda **economiza** o lookup em `/2/users/by` que a leitura de timeline fazia para converter handles em IDs.
- **Autenticação**: o mesmo **bearer OAuth 2.0 App-Only** da busca serve — conferido contra a API real em 2026-08-16 (200, 167 contas na primeira página). Não é preciso contexto de usuário.
- **Rede de segurança**: a lista lida com sucesso é gravada em `.contas_seguidas.json` e, se a X API falhar, é ela que segue valendo — um 503 transitório não pode zerar a pauta depois de o pipeline já ter pago o token. No Render o arquivo **não** sobrevive entre execuções (cada run do cron é um container novo, e o `.json` é gitignored), então lá a falha aborta mesmo.
- **Sem lista nenhuma, aborta**: coletar de zero contas não produz vídeo, só gasta. `X_ACCOUNTS` continua existindo como escape hatch e, quando preenchido, substitui a lista de seguidas inteira.

## Como funciona a régua de engajamento
...
A métrica é o **gancho**: `engagedViews / views` da YouTube Analytics — a fração de quem **abriu o vídeo e ficou**, contra quem deslizou para o próximo. É a tradução direta de "engajamento versus swipe-away", e não deve ser confundida com `averageViewPercentage` (quanto do vídeo quem ficou assistiu), que mede outra coisa e entra só como **desempate**.

O que mudou em `youtube.top_retencao`:

- O ranking **ordena por engajamento** e desempata pela profundidade. Antes os dois eram **multiplicados**, o que deixava um vídeo de gancho fraco e cauda longa empatar com um de gancho forte — exatamente a confusão que a régua nova existe para desfazer.
- O **piso de `ENGAJAMENTO_MINIMO` (70%)** filtra a lista: só sobem os vídeos que seguraram esse percentual ou mais.
- Quando `engagedViews` não está disponível na conta, a Analytics devolve só as métricas clássicas e o gancho fica desconhecido. Aí o piso é aplicado sobre `averageViewPercentage` — não é a mesma coisa, mas é o único sinal de "ficou vs saiu" que sobra; o comportamento antigo (assumir gancho de 100%) faria **todo** vídeo passar no piso.

No prompt de seleção (`escritor._resumo_campeoes`), cada vídeo entra **rotulado em código**: `[ALTO ENGAJAMENTO]` acima do piso, `[abaixo do piso de 70%]` abaixo dele. O rótulo é escrito por Python, e não deixado para o modelo comparar de cabeça, porque regra numérica embutida em prosa é justamente o tipo de instrução que se perde no meio de cem linhas de contexto. O prompt então só precisa dizer o que fazer com ele: **imite os marcados como alto engajamento; trate os de baixo como contraexemplo, por mais views que tenham** — views sem gancho é alcance que o feed empurrou e o espectador recusou.
...
- **Quer mudar as contas acompanhadas** — **siga ou deixe de seguir a conta no próprio X**: a coleta lê a lista de seguidas de `X_USERNAME` a cada execução, e a mudança vale já na próxima. Para um recorte pontual sem mexer em quem você segue, preencha `X_ACCOUNTS` no `.env`.
- **"Não deu para ler as contas que @… segue no X"** — confira `X_USERNAME` (sem `@`, e a conta precisa existir) e as credenciais da X API. Sem lista de seguidas e sem `X_ACCOUNTS` a execução aborta: coletar de zero contas não produz vídeo, só gasta.
